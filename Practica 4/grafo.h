...
class GRAFO {
  ...
  vector<LA_nodo>
      LP;  // Lista de adyacencia de los predecesores: es una matriz dode cada
           // fila posiblemente es posiblemente de distinto tama�o
  ...
  void build(char nombrefichero[85],
             int &errorapertura);  // Crea LS, A y LP en su caso, desde la
                                   // lectura del fichero
  void dfs_num(unsigned i, vector<LA_nodo> L, vector<bool> &visitado,
  ...
  void bfs_num(
      unsigned i, vector<LA_nodo> L, vector<unsigned> &pred,
 ...
 public:
  ...
  //void Mostrar_Lista();
  // void RecorridoProfundidad(); // test
  // un nodo inicial
  void Kruskal();
  void RecorridoProfundidad(unsigned i, vector<LA_nodo> &L, vector<unsigned> &d,
                            vector<bool> &visitado, vector<unsigned> &pred);
