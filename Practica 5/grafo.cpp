...
void GRAFO ::build(char nombrefichero[85], int &errorapertura) {
  ...
  if (textfile.is_open()) {
    unsigned i, j, k;
    textfile >> (unsigned &)n >> (unsigned &)m >>
        (unsigned &)dirigido;  // leo n nodos, m arcos y si es dirigido o no
    // redimensiono lista sucesores y predecesores para que sea igual que n
    LS.resize(n);
    LP.resize(n);
    A.resize(n, std::vector<ElementoLista>(
                    n, {0, maxint}));  // resize o sino peta (matriz de ady nxm)
    for (k = 0; k < m; k++) {          // itero sobre cada arco
      textfile >> (unsigned &)i >> (unsigned &)j >>
          (int &)dummy.c;          // leo costo y nodos del arco
                            ...
                            // enum y postnum
{
  ...
  for (unsigned j = 0; j < L[i].size(); j++)  // cada nodo ady de i
    if (!visitado[L[i][j].j])
      dfs_num(L[i][j].j, L, visitado, prenum, prenum_ind, postnum,
              postnum_ind);  // llamo a dfs
...
void GRAFO::RecorridoProfundidad(unsigned i, vector<LA_nodo> &L,
  ...
  dfs_num(i, LS, visitado, prenum, prenum_ind, postnum,
          postnum_ind);  // recorrido en profundidad desde nodo inicial
  ...
  for (unsigned j = 0; j < prenum_ind; j++) {  // recorro cada num de postorden
    ...
    if (j < prenum_ind - 1)
      cout << " -> ";  // mientras no sea el ultlimo nodo ->
  }
  cout << endl;
...
void GRAFO::bfs_num(unsigned i, vector<LA_nodo> L, vector<unsigned> &pred,
  ...
  while (!cola.empty()) {
    ...
    for (unsigned j = 0; j < L[k].size();
         j++) {  // recorro cada nodo ady al nodo k
      if (!visitado[L[k][j].j]) {
...
void GRAFO::RecorridoAmplitud() {
  vector<unsigned> pred(n,
                        0);  // Vector para almacenar el predecesor de cada nodo
...
void GRAFO::Kruskal() {
  ...
  do {
    ...
    // Para ello recorremos las aristas desde la posicion head+1 hasta la n-1
    // intercambiando la de menor coste con la que esta en la posicion head,
    // salvo que head ya apunte a la ultima arista (la de la posicion n-1)
    if (head < m - 1) {
      unsigned minIndex = head;
      for (unsigned i = head + 1; i < m; i++) {
        // Si el peso de la arista en la posicion head es > que el peso de la
        // arista en la posicion i, entonces intercambiamos Aristas[i] y
        // Aristas[head]
        if (Aristas[i].peso <
            Aristas[minIndex].peso) {  // cambio a > para decreciente pero no me
                                       // toma negativos
          minIndex = i;
        }
      }
    ...
    // miramos si los extremos de la arista en la posicion head estan ccxas
    // distintas, y si es asi, metemos esa arista en el MST fusionando las
    // ccxas, es decir, unificando etiquetas
    if (Raiz[Aristas[head].extremo1] !=
        Raiz[Aristas[head].extremo2]) {  // control de ciclo
      ...
      cout << "Arista número " << cont << " incorporada ("
           << Aristas[head].extremo1 + 1 << ", " << Aristas[head].extremo2 + 1
...
/*
se podra usar en PDM

el tipo deque<unsigned>, que declara una dcola de enteros sin signo
- el método push_back(), que añade un elemento al final de la dcola
- el método push_front(), que añade un elemento al principio de la dcola
- el método empty, que nos indica si la dcola está vacía
- el método front, que nos informa del primer elemento de la dcola
- el método pop_front, que elimina el primer elemento de la dcola
...
/*
siguiendo la teoria de bellman ford
su algoritmo se hace en n-1 iteraciones (n = num de grafos)
si las distancias minimas siguen cambiando significa que hay un ciclo de coste negativo 
ya que siempre intenta mejorar la distancia min
si comprobamos con un n*n que es superior a n-1 podemos detectar si hay o no un ciclo
*/
void GRAFO::PDM() {
    deque<unsigned> dcola; // cola de nodos
    vector<int> d; // vector de distancias min desde nodo origen
    vector<unsigned> pred; // vector de pred 
    vector<bool> Encola; // vector que indica si esta en cola
    unsigned s; // nodo de partida

    // Inicialmente no hay ningún nodo permanentemente etiquetado
    ...
    cout << "Introduce un nodo de partida: " << endl;
    cin >> s;
    s--; // Convertir a índice base 0
    ...
    const unsigned MAX_ITERATIONS = n * n; // limite max de iteraciones
    unsigned iterations = 0;

    while (!dcola.empty() && iterations < MAX_ITERATIONS) { // mientras la cola este vacia y las iteraciones sean mayor que el max de iter
        unsigned k = dcola.front();
        dcola.pop_front(); // sacamos el primer nodo y lo borro
        Encola[k] = false; // Actualizamos Encola después de sacar k de la dcola

        // Recorremos la lista de sucesores que permite identificar los arcos (k, j)
        for (const auto& sucesor : LS[k]) { // recorro los sucesores del nodo k
            unsigned j = sucesor.j; // nodo sucesor
            int ckj = sucesor.c; // coste del arco

            if (d[j] > d[k] + ckj) {
                // Podemos mejorar el camino actual hacia el nodo j pasando por k
                d[j] = d[k] + ckj; // actualizo d min a j
                pred[j] = k; // "" a pred j

                // Si el nodo j nunca ha estado en la cola
                if (pred[j] == UERROR) {
                    dcola.push_back(j); // Insertar al final de la cola
                    Encola[j] = true;
                } else if (!Encola[j]) { // Si ya ha estado en la cola pero no está actualmente
                    dcola.push_front(j); // Insertar al principio de la cola
                    Encola[j] = true;
                }
            }
        }
    ...
    if (iterations == MAX_ITERATIONS) {
        cout << "Hay al menos un circuito de coste negativo en el grafo." << endl;
    } else {
        cout << "Soluciones:" << endl;
        for (unsigned i = 0; i < n; i++) {
            if (i != s) {
                if (d[i] != maxint) {
                    cout << "Camino mínimo desde " << s + 1 << " a " << i + 1 << ": ";
